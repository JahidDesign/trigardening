...
function CartProvider({ children }: { children: React.ReactNode }) {
  ...
  const add = useCallback(
    (id: number) => setItems((s) => (s.includes(id) ? s : [...s, id])),
    []
  );
  const remove = useCallback(
    (id: number) => setItems((s) => s.filter((x) => x !== id)),
    []
  );
  const toggle = useCallback(
    (id: number) =>
      setItems((s) => (s.includes(id) ? s.filter((x) => x !== id) : [...s, id])),
    []
  );
  ...
  const value = useMemo(
    () => ({ items, count: items.length, add, remove, toggle, clear, includes }),
    [items, add, remove, toggle, clear, includes]
  );
  ...
  return (
    <article className="bg-white rounded-2xl shadow-md overflow-hidden hover:shadow-xl transition">
      ...
      <div className="p-4">
        ...
        <div
          className="mt-2 flex items-center gap-1 text-[#E58E26]"
          aria-hidden
        >
        ...
        <div className="max-w-7xl mx-auto px-6">
          <div className="mb-6 flex items-start justify-between gap-4">
            <div>
              ...
              <h1 className="text-2xl md:text-3xl font-extrabold text-[#0E2D1B]">
                Plants
              </h1>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
            <aside className="lg:col-span-3 bg-white rounded-lg shadow p-4 sticky top-24 self-start h-fit">
              <h3 className="font-semibold text-[#0E2D1B] mb-3">
                Filter Products
              </h3>
...
function useCartSafe(): CartState {
  try {
    return useCart();
  } catch {
    return {
